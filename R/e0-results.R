...
get.additional.e0.param <- function(e, ...) 
	return(list(add=c(), plot=c('pi', 'xlim', 'nr.traj'), 
...
show.e0.traj <- function(h, ...) {
	...
	if (show.type == 'plot') {
		param.plot1c <- param.env[add.param.names[['plot']]]
		...
		# Table
		param.table <- param.env[add.param.names[['table']]]
...
e0.show.dl.group <- function(g, main.win, parent.env) {
	...
	dl.f <- gframe("<span color='blue'>DL curve settings</span>", markup=TRUE, 
							horizontal=FALSE, cont=g)
	dlfg1 <- ggroup(horizontal=TRUE, cont=dl.f)
	glabel('CI (%):', cont=dlfg1)
	e$pi <- gedit('80, 95', width=7, cont=dlfg1)
	glabel('Burnin:', cont=dlfg1)
	e$burnin <- gedit(defaults.dl$burnin, width=5, cont=dlfg1)
	glabel('e0 min, max:', cont=dlfg1)
	e$e0.lim <- gedit(defaults.dl$e0.lim, width=7, cont=dlfg1)
	glabel('# curves:', cont=dlfg1)
	e$nr.curves <- gedit(defaults.dl$nr.curves, width=6, cont=dlfg1)
	
	dlfg2 <- ggroup(horizontal=TRUE, cont=dl.f)
	e$predictive.distr <- gcheckbox('Predictive distribution:', 
							checked=defaults.dl$predictive.distr, cont=dlfg2)
