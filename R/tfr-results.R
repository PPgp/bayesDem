TFRresults.group <- function(g, main.win, parent) {
	...
	show.traj.country.f <- gframe("<span color='blue'>Country settings</span>", markup=TRUE, 
									horizontal=FALSE, cont=e$show.traj.g)
	...
	show.traj.tfr.f <- gframe("<span color='blue'>TFR settings</span>", markup=TRUE, 
								horizontal=FALSE, cont=e$show.traj.g)
	show.traj.tfr.g1 <- ggroup(horizontal=TRUE, cont=show.traj.tfr.f)
	glabel('CI (%):', cont=show.traj.tfr.g1)
	e$show.traj.ci <- gedit('80, 95', width=7, cont=show.traj.tfr.g1)
	e$un.low.high <- gcheckbox('+/- 0.5 child', checked=defaults.traj$half.child.variant, 
								cont=show.traj.tfr.g1)
	#show.traj.tfr.g2 <- ggroup(horizontal=TRUE, cont=show.traj.tfr.f)
	addSpace(show.traj.tfr.g1, 15)
	glabel('# trajectories:', cont=show.traj.tfr.g1)
	e$show.nr.traj <- gedit(20, width=6, cont=show.traj.tfr.g1)
	
	show.traj.pred.f <- gframe("<span color='blue'>Time range</span>", markup=TRUE, 
									horizontal=TRUE, cont=e$show.traj.g)
	glabel('From year:', cont=show.traj.pred.f)
	e$traj.from.year <- gedit(mcmc.defaults$start.year, width=4, cont=show.traj.pred.f)
	glabel('To year:', cont=show.traj.pred.f)
	e$traj.to.year <- gedit(defaults.pred$end.year, width=4, cont=show.traj.pred.f)
	...
	e$show.traj.bg <- ggroup(horizontal=TRUE, cont=e$show.traj.g)
	...
	gbutton('Generate Script', cont=e$show.traj.bg, handler=showTFRtraj, 
	...
	e$TableB.show.traj <- gbutton(action=TableB.show.traj.act, cont=e$show.traj.bg)
	gbutton(action=GraphB.show.traj.act, cont=e$show.traj.bg)
	...
	glabel('Percentile:', cont=map.set.g1)
	...
	create.help.button(topic='tfr.map', package='bayesTFR', parent.group=map.bg,
						parent.window=main.win)
	addSpring(map.bg)
	gbutton('Generate Script', cont=map.bg, handler=showMap, 
	...
	dlc.country.f <- gframe("<span color='blue'>Country settings</span>", markup=TRUE, 
							horizontal=FALSE, cont=e$dl.curve.g)
	...
	dlc.dl.f <- gframe("<span color='blue'>DL curve settings</span>", markup=TRUE, 
							horizontal=FALSE, cont=e$dl.curve.g)
	dlc.dl.g1 <- ggroup(horizontal=TRUE, cont=dlc.dl.f)
	glabel('CI (%):', cont=dlc.dl.g1)
	e$dlc.ci <- gedit('80, 95', width=7, cont=dlc.dl.g1)
	#dlc.dl.g2 <- ggroup(horizontal=TRUE, cont=dlc.dl.f)
	glabel('Burnin:', cont=dlc.dl.g1)
	e$dlc.bi <- gedit(defaults.dl$burnin, width=5, cont=dlc.dl.g1)
	glabel('Maximum TFR:', cont=dlc.dl.g1)
	e$dlc.tfr.max <- gedit(defaults.dl$tfr.max, width=2, cont=dlc.dl.g1)
	glabel('# curves:', cont=dlc.dl.g1)
	e$dlc.nr.curves <- gedit(20, width=6, cont=dlc.dl.g1)
	...
	e$dlc.bg <- ggroup(horizontal=TRUE, cont=e$dl.curve.g)
	create.help.button(topic='DLcurve.plot', package='bayesTFR', parent.group=e$dlc.bg,
						parent.window=main.win)
	addSpring(e$dlc.bg)
	gbutton('Generate Script', cont=e$dlc.bg, handler=showDLcurve, 
								action=list(mw=main.win, env=e, script=TRUE))
	addSpace(e$dlc.bg, 5)
	GraphB.dlc <- gaction(label='Graph', icon='lines', handler=showDLcurve, 
						action=list(mw=main.win, env=e, script=FALSE))
	gbutton(action=GraphB.dlc, cont=e$dlc.bg)
	
	############################################
	# Parameter Traces
	############################################
	traces.g <- ggroup(label="<span color='#0B6138'>Parameter Traces</span>", markup=TRUE, horizontal=FALSE, cont=e$nb)
	traces.g2 <- ggroup(horizontal=TRUE, cont=traces.g)
	glabel('Parameters:', cont=traces.g2)
	e$traces.pars.chb <- gcheckbox("all", cont=traces.g2, checked=TRUE, 
							handler=function(h,...) 
	...
	e$traces.par.dl <- gdroplist(tfr.parameter.names(), cont=traces.g2)
	enabled(e$traces.par.dl) <- FALSE
	e$traces.par.cs.dl <- gdroplist(tfr.parameter.names.cs(), cont=traces.g2)
	enabled(e$traces.par.cs.dl) <- FALSE
	addSpace(traces.g2, 10)
	glabel('# points:', cont=traces.g2)
	e$traces.nr.points <- gedit(100, width=5, cont=traces.g2, coerce.with=as.numeric)
	
	traces.g1 <- ggroup(horizontal=TRUE, cont=traces.g)
	e$traces.cs.chb <- gcheckbox("Country specific", checked=FALSE, cont=traces.g1,
							handler=function(h,...) {
								if (svalue(h$obj)) {
	...
	addSpring(traces.g)
	traces.bg <- ggroup(horizontal=TRUE, cont=traces.g)
	...
	gbutton(action=GraphB.traces, cont=traces.bg)
...
create.country.widget <- function(parent, defaults=NULL, main.win=NULL, show.all=TRUE, 
	...
	g1 <- ggroup(horizontal=TRUE, cont=parent)
	glabel('Country:', cont=g1)
	if(show.all) {glabel("<span color='red'>(*)</span>", markup=TRUE, cont=g1)}
	else glabel("<span color='red'>*</span>", markup=TRUE, cont=g1)
	e$country.w <- gedit(width=20, cont=g1)
	e$country.select.b <- gbutton('Select', cont=g1, handler=selectCountryMenu,
								action=list(mw=main.win, text.widget=e$country.w, env=e))
	if(show.all) {
		g2 <- ggroup(horizontal=TRUE, cont=parent)
		e$all.countries.chb <- gcheckbox('All countries', checked=FALSE, cont=g2,
									handler=function(h,...){
		...
		addSpace(g2, 15)
		glabel("Output type:", cont=g2)
		e$all.type <- gdroplist(c('png', 'jpeg', 'pdf', 'tiff', 'bmp', 'postscript'), cont=g2)
		enabled(e$all.type) <- FALSE
		g3 <- ggroup(horizontal=TRUE, cont=parent)
		addSpace(g3,7)
		glabel("Output directory:", cont=g3)
		e$all.output <- gfilebrowse(eval(defaults$output.dir), type='selectdir', 
					  width=39, quote=FALSE, cont=g3)
...
create.graph.pars.widgets <- function (parent, main.win=NULL) {
	g <- ggroup(horizontal=FALSE, cont=parent)
	glabel("Comma-separated parameters in R format (see 'help(par)'), e.g. ylim=c(0,6), xlab='Time'", cont=g)
	pars.w <- gedit('', width=40, cont=g)
	return(pars.w)
}
...
selectCountryMenu <- function(h, ...) {
	...
	if(new.window) {
		...
		h$action$env$selcountry.gt <- gtable(h$action$env$country.table, cont=t.group, expand=TRUE,				handler=country.selected)
		b.group <- ggroup(horizontal=TRUE, cont=t.group)
		gbutton('Cancel', cont=b.group, handler=function(h, ...) 
					visible(win) <- FALSE)
		addSpring(b.group)
		h$action$env$selcountry.okbutton <- gbutton('OK', cont=b.group)
	}
...
showConvergenceDiag <- function(h, ...) {
	...
	glabel(paste('There ', verb, ' ', ldiag, ' set', noun.postfix, ' of diagnostics.', sep=''),
			cont=g)
	glabel('Click on the traffic lights to get a report.', cont=g)
	addSpace(g, 10)
	glo <- glayout(cont=g)
	glo[4,1] <- glabel('Needs at least', cont=glo)
	l <- 1
	for(i in 1:ldiag) {
		diag <- diag.all[[i]]
		...
		glo[l,i+1] <- glabel(paste('Burnin =', diag$burnin), cont=glo)
		glo[l+1,i+1] <- glabel(paste('Thin =', diag$thin), cont=glo)
		glo[l+2,i+1] <- gimage(image.name, cont=glo, handler=showDiagInfo, 
								action=list(mw=win, env=e, diag=diag))
		glo[l+3,i+1] <- glabel(diag$iter.needed, cont=glo)
	}
	glo[4,ldiag+2] <- glabel('additional iterations', cont=glo)
	addSpace(g,20)
}
